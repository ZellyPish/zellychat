...
const InputComponent = React.forwardRef(({ label, name, type, value, auto = '', onChange }, ref) => (
  <React.Fragment>
    ...
    <input type={type} name={name} id={name} autoComplete={auto} css={tw`h-10 px-4 mt-1 focus:ring-purple-500 focus:border-purple-500 block w-full shadow-sm sm:text-sm border border-gray-300 rounded-md`} value={value} onChange={onChange} ref={ref} />
  </React.Fragment>
))
